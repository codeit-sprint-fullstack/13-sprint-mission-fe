...
export default function LikeButton({
  ...
  const toggleLikeMutation = useMutation<Product, Error, void>({
    mutationFn: () =>
      isFavorite
        ? productService.unlike(productId)
  ...
  return (
    <button
      type="button"
      className={`flex h-10 cursor-pointer items-center gap-1.5 rounded-[2.1875rem] border border-solid px-4 py-1 transition-all ${
        isFavorite
    ...
    >
      ...
      >
        <path
          ...
          stroke={isFavorite ? "#F472B6" : "#6B7280"}
          strokeWidth="1.8"
        />
      </svg>
      <span
        className={`text-[1rem] font-medium ${isFavorite ? "text-pink-400" : "text-cool-gray-500"}`}
      >
        {favoriteCount}
      </span>
    </button>
