...
function omitEmptyParams(params: Record<string, unknown>): Record<string, unknown> {
...
axios.interceptors.response.use(
  ...
  async function (error: AxiosError<ErrorResponse>) {
    ...
    if (
      ...
      try {
        const response = await axiosLib.post<TokenResponse>(`${API_BASE_URL}/auth/refresh-token`, {
          refreshToken,
        });
