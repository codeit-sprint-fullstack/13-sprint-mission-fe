...
import type { Article, Comment, PaginatedResponse } from "@/types/api";
...
export default function ArticleDetailPage() {
  ...
  const toggleLike = useMutation<Article, unknown, void>({
    mutationFn: () =>
  ...
  const createComment = useMutation<Comment, unknown, void>({
    ...
    <div className="bg-white text-[#1f2937]">
      ...
      >
        {articleQuery.isLoading ? (
          <p className="rounded-lg bg-[#f3f4f6] p-[18[x] text-[#6b7280]">
            게시글 불러오는 중...
          </p>
        ) : null}
        ...
        {article ? (
          <>
            <article className="border-b border-[#e5e7eb] pb-8">
              ...
              <p className="mt-7 leading-[1.7] whitespace-pre-line text-gray-700">
                {article.content}
              </p>
