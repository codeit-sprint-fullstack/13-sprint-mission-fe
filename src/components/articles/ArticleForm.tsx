...
export default function ArticleForm({
  ...
  defaultValue?: Article | null;
  articleId?: string | number | null;
}) {
  const router = useRouter();
  const [error, setError] = useState<string | null>(null);
  const [isSubmitted, setIsSubmitted] = useState<boolean>(false); // 제출 완료 상태
  ...
  const { mutate: submitArticle, isPending: isLoading } = useMutation({
    mutationFn: (payload: ArticlePayload) =>
      isEditMode
        ? updateArticleAction(articleId as string | number, payload)
