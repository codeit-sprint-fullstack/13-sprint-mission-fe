...
import type { PaginatedResponse, Product, SortOrder } from "@/types/api";
...
export default function ItemsPage() {
  ...
  const bestProductsQuery = useQuery<PaginatedResponse<Product>>({
    ...
    <div className="bg-white text-[#1f2937]">
      ...
      >
        <section>
          ...
          <div className="tablet:grid-cols-2 desktop:grid-cols-4 desktop:gap-x-5 desktop:gap-y-9 grid grid-cols-2 gap-x-3.5 gap-y-7">
        ...
        <section className="mt-11">
          <div className="desktop:flex-row desktop:items-center desktop:justify-between mb-[22px] flex flex-col items-stretch gap-6">
            <h2 className="desktop:mb-0 mb-[18px] text-xl font-bold">
              판매 중인 상품
            </h2>
            <div className="desktop:flex-row desktop:items-center flex flex-col items-stretch gap-3">
              <form
                className="desktop:w-[min(320px,32vw)] relative h-[42px] w-full rounded-lg bg-[#f3f4f6] text-gray-400"
                onSubmit={search}
              >
                <Search
              ...
              <label className="inline-fles desktop:w-auto relative w-full items-center">
                <select
                  ...
                  value={orderBy}
                  onChange={(event) => {
                    (setPage(1), setOrderBy(event.target.value as SortOrder));
                  }}
                >
