...
export default function ItemDetailPage() {
  ...
  const commentsQuery = useQuery<PaginatedResponse<Comment>>({
    queryKey: queryKeys.comments(productId),
    queryFn: () => commentApi.list(productId),
    enabled: Boolean(productId),
  });

  const updateProductMutation = useMutation<Product, unknown, ProductPayload>({
  ...
  const deleteProductMutation = useMutation<{ ok: boolean }, unknown, void>({
  ...
  const favoriteMutation = useMutation<Product, unknown, void>({
    mutationFn: () => {
  ...
  const deleteCommentMutation = useMutation<{ ok: boolean }, unknown, string>({
  ...
  return (
    <div className="bg-white text-[#1f2937]">
      <Header />
      <main
        className={`${shellClass} min-h-[calc(100vh-17px)] pt-[34px] pb-[130px]`}
      >
        ...
        {product ? (
          <>
            <section className="desktop:grid-cols-[minmax(340px,486px)_1fr] grid grid-cols-1 gap-7 border-b border-[#e5e7eb] pb-8">
              ...
              <article className="min-w-0">
                <div className="flex items-start justify-between gap-4 border-b border-[#e5e7eb] pb-[18px]">
                  <div>
                    ...
                    <strong className="desktop:text-[48px] text-[32px]">
                  ...
                  <div className="relative">
                    ...
                    {productMenuOpen ? (
                      <div className="absolute top-full right-0 z-[5] w-[120px] overflow-hidden rounded-lg border border-[#e5e7eb] bg-white shadow-xl">
                        <button
                ...
                <div className="mt-6">
                  ...
                  <p className="leading-[1.7] whitespace-pre-line text-gray-600">
                    {product.description}
                  </p>
                </div>
                ...
                <div className="desktop:flex-row desktop:items-center mt-[42px] flex flex-col items-start justify-between gap-[18px]">
            ...
            <section className="pt-[30px]">
              ...
              >
                ...
                />
                <button
                  className="inline-flex h-[42px] min-w-[74px] items-center justify-center justify-self-end rounded-lg bg-gray-400 px-[18px] text-[16px] font-bold text-white enabled:bg-[#3692ff] disabled:cursor-not-allowed disabled:opacity-70"
                  type="submit"
              ...
              <div className="mt-[18px]">
                {commentsQuery.data?.list.map((item) => (
                  <article
                    className="relative border-b border-[#e5e7eb] py-5 pr-11 pl-0"
                    key={item.id}
                  >
