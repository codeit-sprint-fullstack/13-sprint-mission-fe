...
type CommentMenuAction = "edit" | "delete";
...
function ArticleCommentSection({ articleId }: ArticleCommentSectionProps) {
  const { user } = useAuth();
  ...
  const commentsQuery = useInfiniteQuery({
    ...
    queryFn: ({ pageParam }: { pageParam: number | undefined }) =>
      getArticleComments(articleId, { cursor: pageParam }),
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, value }: UpdateCommentVariables) => patchComment(id, { content: value }),
          ...
          <CommentHeader>
            ...
            {user?.id === comment.writer.id && (
              <ToggleMenu
                ...
                onSelect={({ value }: MenuOption<CommentMenuAction>) => {
                  if (value === "edit") {
