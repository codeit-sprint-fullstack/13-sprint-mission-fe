...
interface AuthContextValue {
  user: User | null;
  signup: (formData: SignUpInput) => Promise<AuthResult>;
  signin: (formData: SignInInput) => Promise<AuthResult>;
  signout: () => void;
  isPending: boolean;
}
...
function useTokensFromParams(onTokensReceived: () => void): void {
...
export const AuthProvider = ({ children }: PropsWithChildren) => {
  ...
  const handleAuthSuccess = useCallback(
    ({ user: authenticatedUser }: AuthResult) => {
  ...
  const signup = useCallback(
    (formData: SignUpInput) => signupMutation.mutateAsync(formData),
    [signupMutation]
  );

  const signin = useCallback(
    (formData: SignInInput) => signinMutation.mutateAsync(formData),
    [signinMutation]
  );
  ...
  return (
    <AuthContext.Provider
      value={{
        ...
        signin,
        signout,
        isPending: hasAccessToken && isPending,
      }}
    >
      {children}
    </AuthContext.Provider>
